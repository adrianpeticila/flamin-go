/**
 * M2M Agentic Commerce Engine for Flamin.go
 * Implements exact 7-key catalog schema, rate limiting, idempotency,
 * x402 programmatic micropayment rails, Stripe hosted checkout, and HMAC-SHA256 webhooks.
...
export const CATALOG = [
  {
    id: "ad-concept-pack",
    name: "Ad Concept Pack",
    price_cents: 9900,
    currency: "USD",
    description: "High-velocity ad concept generation system delivering creative angle variations, hook scripts, and audience targeting parameters for rapid performance testing.",
    sample_output_url: "https://flamin-go.pages.dev/samples/ad-concept-pack.json",
    checkout_type: "stripe_hosted"
  },
  {
    id: "visual-hook-matrix",
    name: "Visual Hook Matrix",
    price_cents: 4900,
    currency: "USD",
    description: "Structured visual hook library and retention scoring matrix designed for automated video and image ad creative synthesis pipelines.",
    sample_output_url: "https://flamin-go.pages.dev/samples/visual-hook-matrix.json",
    checkout_type: "stripe_hosted"
  },
  {
    id: "micro-hook-query",
    name: "Micro Hook Query",
    price_cents: 500,
    currency: "USD",
    description: "Programmatic single-query visual hook scoring and real-time creative angle generation over x402 micropayment rail.",
    sample_output_url: "https://flamin-go.pages.dev/samples/micro-hook-query.json",
    checkout_type: "x402"
  }
];
...
export const CORS_HEADERS = {
  ...
  "Access-Control-Allow-Headers": "Content-Type, Authorization, Idempotency-Key, X-Payment-Proof, X-Signature-SHA256, X-Signature, X-Webhook-Signature, Stripe-Signature",
...
const fallbackMemoryStore = new MemoryKV();
...
export async function createHmacSignature(rawBody, secret) {
...
export async function verifyHmacSignature(rawBody, signature, secret) {
  ...
  try {
    const expectedHex = await createHmacSignature(rawBody, secret);
...
export async function handleBuyRequest(request, env) {
  ...
  if (product.checkout_type === "x402") {
    ...
    const checkoutUrl = env?.STRIPE_CHECKOUT_URL ||
      `https://buy.stripe.com/4gM5kwbA75w0avzdjI5AQ00?session=${checkoutSessionId}&product=${product.id}`;
...
export async function handleWebhookRequest(request, env) {
  ...
  const secret = env?.PAYMENT_WEBHOOK_SECRET ||
    env?.WEBHOOK_SECRET ||
    "default_nyx_webhook_secret_flamingo";
  ...
  const isProgrammatic = payload.rail === "x402" ||
    ...
    payload.product_id === "micro-hook-query";
  ...
  return jsonResponse({
    ...
    rail: payload.rail || (isProgrammatic ? "x402" : "stripe_hosted"),
